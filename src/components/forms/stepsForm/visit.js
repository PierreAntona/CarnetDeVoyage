...
export default function VisitForm({ startDate, user, destination, setIsOpen }) {
    ...
    const addStep = async () => {
        ...
        if (name !== "") {
            await setDoc(doc(docRef, "planning", `visit-${name}`), {
                type: "Visite",
                name: name,
                time: time,
                date: date,
    ...
    return (
        <>
            ...
            <DatePicker
                ...
                mode="time"
                title="Selectionner une heure de visite"
                confirmText="Confirmer"
                cancelText="Annuler"
                theme="light"
            />
